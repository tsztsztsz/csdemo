...
/* ========== API functions ================ */
int cs_tpiu_pattern_test(cs_device_t dev) {
	struct cs_device *d = DEV(dev);
	assert(cs_device_has_class(dev, CS_DEVCLASS_SINK));
	_cs_unlock(d);
	if (d->type == DEV_TPIU) {
		_cs_write(d, CS_TPIU_TPRCR, 0xF); // run each pattern 16 times
		_cs_write(d, CS_TPIU_CUR_TEST_PAT_MODE, CS_TPIU_ALL_PATTERN_TIME);
		usleep(100);
		_cs_write(d, CS_TPIU_TPRCR, 0x0);
		_cs_write(d, CS_TPIU_CUR_TEST_PAT_MODE, 0x0);
		return 0;
	} else {
		return -1;
	}
...
int cs_etf_enable(cs_device_t dev) {
	struct cs_device *d = DEV(dev);
	unsigned int current_TMC_mode;
	unsigned int new_TMC_mode;
	assert(cs_device_has_class(dev, CS_DEVCLASS_SINK));

	_cs_unlock(d);
	if (d->type == DEV_ETF) {
		unsigned int ffcr, pscr;
		d->v.etb.currently_reading = 0;
		if (_cs_isset(d, CS_ETB_CTRL, CS_ETB_CTRL_TraceCaptEn)) {
			printf("CSETF: ETF is already running");
			return -1;
		}
		/*
		 * wait until TMC_READY bit is set.
		 */
		while (!_cs_isset(d, CS_ETB_STATUS, CS_TMC_STATUS_TMCReady))
			;
		diagf("TMC is ready.\n");

		current_TMC_mode = _cs_read(d, CS_TMC_MODE);
		/*
		 * set TMC mode to HW FIFO
		 */
		_cs_write(d, CS_TMC_MODE, CS_TMC_MODE_HWFIFO);
		new_TMC_mode = _cs_read(d, CS_TMC_MODE);
		diagf("TMC mode [0: CIRC, 1: SWFIFO, 2: HWFIFO] is set from %i to %i.\n", current_TMC_mode, new_TMC_mode);
		/* periodic synchronization register: period=2^PSCOUNT     0xA => 1024Bytes   */
		pscr = 0x0; // no sync
		_cs_write(d, CS_ETB_PSCR, pscr);
...
//		ffcr = 0x0; // disable formating.. does this work for TMC devices in HWFIFO mode????
		...
		if (d->v.etb.is_tmc_device) {
			...
			ffcr |= CS_ETB_FLFMT_CTRL_StopFl;
		}
		_cs_set(d, CS_ETB_FLFMT_CTRL, ffcr);
...
int cs_etf_disable(cs_device_t dev) {
	struct cs_device *d = DEV(dev);
	assert(cs_device_has_class(dev, CS_DEVCLASS_SINK));

	/*
	 * only disable when trace session is over, for example because trace sink has stopped.
	 */
	_cs_unlock(d);
	if (d->type == DEV_ETF) {
		unsigned int flfmt;
		...
		/*
		 * set 'stop on flush' and 'flush manual' bits in FFCR
		 */
		flfmt = CS_ETB_FLFMT_CTRL_StopFl | CS_ETB_FLFMT_CTRL_FOnMan;
		_cs_set(d, CS_ETB_FLFMT_CTRL, flfmt);
		diagf("ETF is configured to 'stop on flush' and 'flush manual'\n");

		/*
		 * wait until TMC_READY bit is set.s
		 */
		while (!_cs_isset(d, CS_ETB_STATUS, CS_TMC_STATUS_TMCReady))
			;
		diagf("ETF is now empty.\n");
...
int cs_etr_axi_enable(cs_device_t dev) {
	...
	if (d->type == DEV_ETB || d->type == DEV_ETF) {
		...
		/* RAM DEPTH (RSZ) = 512MB --> 32-Bit-word:0x8000000*/
		_cs_write(d, CS_ETB_RAM_DEPTH, 0x8000000);
		/* set TMC mode to Circular Buffer*/
		_cs_write(d, CS_TMC_MODE, CS_TMC_MODE_CIRCULAR);
		/* AXI Control
		 max. of 2  data transfers per burst.
		 scatter-gather mode disabled.
		 cache enabled on writes & reads, non-secure access, privileged access.
		 */
		_cs_clear(d, CS_TMC_AXICTL, CS_TMC_SCATGAT_MODE);
		_cs_set(d, CS_TMC_AXICTL, CS_TMC_AXICTL_WRBURSTLEN128 | CS_TMX_AXICTL_CACHEPROT);
		/* set 32-Bit address to locate trace buffer in system memory*/
		_cs_write(d, CS_TMC_DBALO, 0x60000000);
		/* set high part of the address of the trace buffer in system memory*/
		_cs_write(d, CS_TMC_DBAHI, 0x00);

		/* periodic synchronization register: period=2^PSCOUNT     0xA => 1024Bytes   */
		_cs_write(d, CS_ETB_PSCR, 0x0);
		/* Set up flushing and formatting controls.
		 CS_ETB_FLFMT_CTRL_EnFTC: enable formatting into 16-byte frames,
		 in case there are multiple trace sources.
		 CS_ETB_FLFMT_CTRL_EnFCont: enable continuous formatting (ETB)
		...
		/* Stop on a Flush operation.  For a TMC ETB we don't want to go straight
		 ...
		 then read the data, then disable for reprogramming.
		 */
		_cs_set(d, CS_ETB_FLFMT_CTRL, CS_ETB_FLFMT_CTRL_EnFTC | CS_ETB_FLFMT_CTRL_EnTI | CS_ETB_FLFMT_CTRL_StopFl);
		return _cs_write(d, CS_ETB_CTRL, CS_ETB_CTRL_TraceCaptEn);
	} else {
		/* The only other sinks would be trace ports, and currently this
		 library doesn't support use cases which have an external
...
//        flfmt=0x0;
		...
		unsigned int ffcr, fscr;
		/* check if formatter is stopped */
		rc = _cs_wait(d, CS_TPIU_FLFMT_STATUS, CS_TPIU_FLFMT_STATUS_FtStopped);
		if (rc != 0) {
			return rc;
		}
		/* enable traceclock from PL (not from PS clock controller) */
		_cs_set(d, CS_TPIU_EXTCTL_OUT_PORT, CS_TPIU_EXTCTL_OP_TCLK);
		/* formatter synchronization counter*/
		fscr = 0x100; // insert a full sync frame every 4096 (0xFFF)frames (=65536 bytes) , 4KB --> 256 frames --> 0x100
		_cs_set(d, CS_TPIU_FSCR, fscr);
		diagf("formatter synchronization counter: sync every %8d frames\n", _cs_read(d, CS_TPIU_FSCR) + 1);
		/* flush & formatter control register*/
		ffcr = _cs_read(d, CS_TPIU_FLFMT_CTRL);
		//ffcr |= CS_TPIU_FLFMT_CTRL_EnFCont; /* enable continuous formatting, embed in trigger packets, indicate null cycles using sync packets */
		ffcr = 0x0;
		ffcr |= CS_TPIU_FLFMT_CTRL_EnFTC; /* enable formatting */
		rc = _cs_write(d, CS_TPIU_FLFMT_CTRL, ffcr);
		if (rc != 0) {
			return rc;
		}
		diagf("formatter enabled:            %i\n", _cs_isset(d, CS_TPIU_FLFMT_CTRL, CS_TPIU_FLFMT_CTRL_EnFTC));
		diagf("continuos formatting enabled: %i\n", _cs_isset(d, CS_TPIU_FLFMT_CTRL, CS_TPIU_FLFMT_CTRL_EnFCont));
		return 0;
	} else {
		/* The only other sinks would be trace ports, and currently this
		 library doesn't support use cases which have an external
...
int cs_sink_disable(cs_device_t dev) {
	...
	if (d->type == DEV_TPIU) {
		...
		/* ETB or TMC */
		if (d->v.etb.is_tmc_device && _cs_isset(d, CS_ETB_CTRL, CS_ETB_CTRL_TraceCaptEn)) {
...
int cs_get_buffer_unread_bytes(cs_device_t dev) {
	...
	if (d->v.etb.finished_reading) {
		...
		if (rdptr <= wrptr) {
			...
			unread = d->v.etb.buffer_size_bytes - (rdptr << shift) + (wrptr << shift);
		}
	}
	return unread;
}
...
int cs_get_trace_data(cs_device_t dev, void *buf, unsigned int size) {
	...
	if (DTRACE(d)) {
		diagf("!ctrl=%08X status=%08X flstatus=%08X readptr=%08X writeptr=%08X unread=%04X\n", _cs_read(d, CS_ETB_CTRL), _cs_read(d, CS_ETB_STATUS),
				...
				CS_ETB_RAM_RD_PTR), _cs_read(d, CS_ETB_RAM_WR_PTR), unread);
	}
	...
	if (0) {
		fprintf(stderr, "TraceCaptEn=%u TMCReady=%u Empty=%u CBUFLEVEL=0x%x\n", _cs_isset(d, CS_ETB_CTRL, CS_ETB_CTRL_TraceCaptEn),
				_cs_isset(d, CS_ETB_STATUS, CS_TMC_STATUS_TMCReady), _cs_isset(d, CS_ETB_STATUS, CS_TMC_STATUS_Empty), _cs_read(d, CS_TMC_CBUFLEVEL));
	}
	...
	if (cs_buffer_has_wrapped(dev)) {
		...
		do {
			...
			if (retries > 3) {
				...
				return cs_report_device_error(d, "can't reset the wrapped flag, status=%08X, fl.status=%08X", status, flstat);
			}
...
int cs_insert_trace_data(cs_device_t dev, void const *buf, unsigned int size) {
	...
	if (DTRACE(d)) {
		diagf("  ctrl=%08X status=%08X flstatus=%08X writeptr=%08X\n", _cs_read(d, CS_ETB_CTRL), _cs_read(d, CS_ETB_STATUS),
				_cs_read(d, CS_ETB_FLFMT_STATUS), _cs_read(d, CS_ETB_RAM_WR_PTR));
	}
